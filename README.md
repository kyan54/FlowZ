...
- **连接历史**：可选记录域名/IP、进程、命中规则、实际出口和流量，支持自定义起止时间、仅代理/仅直连筛选、1/3/7 天保留和一键清除；默认关闭，不记录请求内容或源 IP。
...
- 连接拓扑 · 实时流量统计与测速 · 可选连接历史 · 出口 IP 展示
...
### 连接诊断 · 实时日志
逐连接速率 / 规则命中 / 节点链；可选按时间查询域名/IP 的代理历史；分级实时日志：
